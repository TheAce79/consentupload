    ...
    public static void SaveToCsv(IEnumerable<ClinicPdfClientRecord> records, string targetCsvPath)
    {
        ...
        try
        {
            ...
            }))
            {
                ...
                csv.WriteField("ClientIdStatus"); csv.WriteField("FirstName"); csv.WriteField("LastName"); csv.WriteField("MiddleName"); csv.NextRecord();
                foreach (ClinicPdfClientRecord record in records)
                {
                    ...
                    csv.WriteField((int)record.ClientIdStatus); csv.WriteField(record.FirstName); csv.WriteField(record.LastName); csv.WriteField(record.MiddleName); csv.NextRecord();
                }
            }
