...
public static class CsvExporterService
{
    ...
    [
        ("Booking ID", r => r.BookingId), ("Clinic Name", r => r.ClinicName),
    ...
    public static void SaveToCsv(IEnumerable<ClinicPdfClientRecord> records, string targetCsvPath)
    {
        ...
        try
        {
            ...
            }))
            {
                ...
                csv.WriteField("ClientIdStatus"); csv.WriteField("FirstName"); csv.WriteField("LastName"); csv.WriteField("MiddleName"); csv.WriteField("ErrorDetails"); csv.WriteField("BestMatch"); csv.WriteField("Phone"); csv.WriteField("Email"); csv.WriteField("VaccineType"); csv.WriteField("Clinic Date");
                ...
                foreach (ClinicPdfClientRecord record in recordList)
                {
                    ...
                    csv.WriteField((int)record.ClientIdStatus); csv.WriteField(record.FirstName); csv.WriteField(record.LastName); csv.WriteField(record.MiddleName); csv.WriteField(record.ErrorDetails); csv.WriteField(record.BestMatch); csv.WriteField(record.Phone); csv.WriteField(record.Email); csv.WriteField(record.VaccineType); csv.WriteField(record.ClinicDate);
